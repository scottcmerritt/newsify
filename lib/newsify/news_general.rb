...
module NewsGeneral
  ...
  def get_links active=nil
    ...
    #@menu_links.push({label:"Sources",href:newsify.sources_path,count:Source.count}) #if defined?(Source) #defined?(newsify.sources_path) && defined?(Source)
    @menu_links.push({label:"Summaries",href:newsify.summaries_path,count:Newsify::Summary.count})
    @menu_links.push({label:"Being discussed",href:newsify.items_path,count:Newsify::Item.count})
